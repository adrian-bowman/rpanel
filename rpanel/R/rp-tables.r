...
tables_ui <- shiny::fluidPage(
  shiny::titlePanel("Distributions"),
  shiny::sidebarLayout(      
    shiny::sidebarPanel(
      shiny::radioButtons("distribution", "Distribution", c("normal", "t", "chi-squared", "F")),
      shiny::numericInput("degf1", "df1", 1, 1),
      shiny::numericInput("degf2", "df2", 1, 1),
      shiny::checkboxInput("observed.value.showing", "Show observed value"),
      shiny::numericInput("xobs.prob.1", "Observed value", 0),
      shiny::numericInput("xobs.prob.2", "Fixed probability, p", 0.05),
      shiny::radioButtons("tail.area", "Tail area", c("none", "from observed value", "fixed probability")),
      shiny::radioButtons("tail.direction", "Tail direction", c("lower", "upper", "two-sided"))
    ),
    shiny::mainPanel(
...
tables_server <- function(input, output) {
  tables.panel <- reactive({
                    list(distribution = input$distribution,
                         degf1 = input$degf1, degf2 = input$degf2,
                         xobs.prob = c(input$xobs.prob.1, input$xobs.prob.2),
                         observed.value.showing = input$observed.value.showing,
                         tail.area = input$tail.area, tail.direction = input$tail.direction,
                         missing.heading = TRUE)})
  output$plot <- renderPlot(tables.draw(tables.panel()))
}
