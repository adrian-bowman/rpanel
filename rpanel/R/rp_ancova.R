...
rp.ancova.new <- function(x, y, group, panel = TRUE, panel.plot = TRUE,
   ...
   rp.ancova.draw <- function(panel) {
      ...
      dfrm        <- data.frame(x, z)
      names(dfrm) <- panel$term.names[1:2]
      ...
      if (style == "ggplot") {
         dfrm <- data.frame(x, y, group, fvals = fitted(mdl1))
         plt  <- ggplot2::ggplot(dfrm, ggplot2::aes(x, y, group = group, col = group)) +
                   ggplot2::geom_point() +
                   ggplot2::labs(x = xlab, y = ylab, col = glab)
         if (any(panel$model)) {
   ...
   rp.ancova.effectsplot <- function(panel) {
      with(panel, {
         if (model.check & model0.check & any(model) & any(model0) & !all(model == model0))
            print(rp.regression(mdl1, labels = names(mdl1$coefficients)[-1]))
         else {
            par(mar = c(0, 0, 0, 0) + 0.1, bg = bgdcol)
            plot(c(0, 1), c(0, 1), type = "n", xlab = "", ylab = "", axes = FALSE)
         }
   ...
   if (panel) {
      ...
      rp.grid(panel, "controls", row = 0, column = 0, background = bgdcol)
      rp.grid(panel, "models", grid = "controls", row = 0, column = 0, background = bgdcol)
      rp.grid(panel, "fplot",  grid = "controls", row = 1, column = 0, background = bgdcol)

      if (panel.plot) {
         rp.grid(panel, "dataplot", row = 0, column = 1, background = "white")
         ...
         rp.tkrplot(panel, fplot, rp.ancova.effectsplot,
